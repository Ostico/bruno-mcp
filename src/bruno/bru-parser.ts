...
// The `.bru` environment dialect is a separate module, re-exported here because
// every caller reaches it through this one.
export {
  parseBruEnvironment,
  parseBruEnvironmentRaw,
  parseBruEnvironmentFile,
  generateBruEnvironment,
  generateBruEnvironmentFull,
} from './bru-environment.js';
...
function readRequestSettings(value: unknown): BruRequestSettings | undefined {
  ...
  // Kept at 0 for the same reason, and unlike upstream: `bruToJsonV2` reads this
  // key under a truthiness guard, so a `.bru` file saying "never ping" comes back
  // from Bruno's own reader with no key at all. 0 and an absent key mean the same
  // thing to a session, and keeping the value is what lets a write reproduce the
  // file it was read from.
  if (typeof raw.keepAliveInterval === 'number') {
    settings.keepAliveInterval = raw.keepAliveInterval;
  }
