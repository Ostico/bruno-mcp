...
// SECURITY: verbatim
const GROUP_ISOLATION =
  'Each group owns its OWN variable store and cookie jar, so nothing a group sets — a bru.setVar, a ' +
    'session cookie — is visible to any other group. That is what makes running the same five requests as ' +
    'alice and as bob in one call safe.';

// SECURITY: verbatim
const RESPONSE_HEADER_MASKING =
  'credential-named values are masked, and set-cookie is a LIST — one entry per cookie — whose entries ' +
    'keep every attribute (HttpOnly, Secure, SameSite, Path, Max-Age) and withhold only the cookie value, ' +
    'so a cookie-flag or HSTS check is one call.';

// SECURITY: verbatim
const SSRF_CONTRACT =
  'Outbound requests are SSRF-filtered: targets resolving to private, loopback, link-local or otherwise ' +
    'reserved addresses are refused unless the server operator has allowlisted them, and a refusal is ' +
    'reported per-request as an "SSRF blocked" error with status 0.';

const RUN_COLLECTION_DESCRIPTION = [
  'Execute requests in a Bruno collection and run their test scripts.',
  'SUBSET: requests is an ORDERED list of .yml/.bru paths, absolute or relative to collectionPath; a ' +
    'directory expands recursively. Duplicates allowed, and run twice. Omit to run the whole collection. ' +
  ...
  `RESPONSE DATA: every result carries the response body (response_body, response_content_type, ` +
    ...
    `result also carries response_headers, no test script needed; includeResponseHeaders false omits them, ` +
    `maxResponseHeaderBytes bounds one value, and includeResponseBody false bounds the body only, never ` +
    `the headers: ${RESPONSE_HEADER_MASKING} A websocket result's response_headers hold its handshake ` +
    `response (the 101), the only place a session cookie or a negotiated sec-websocket-protocol is visible ` +
    `for that transport; a gRPC result reports metadata under grpc instead.`,
  'ORDER: within a group, the order given; a directory expands by seq scoped to its own folder, ' +
    'subfolders before that folder\'s loose requests, ties by filename. An unparseable file is skipped ' +
    'when DISCOVERED (whole collection, or directory expansion): parseErrors counts them, parseFailures ' +
    'names each with its reason. A file you NAMED is not skipped: it fails the group that named it, ' +
    'reported as that group\'s error, and other groups still run. A named request that does not exist goes ' +
    'to that group\'s missingRequests and fails nothing.',
  'TRANSPORTS: http, graphql, grpc, websocket. Any other kind is refused per-request with status 0 and a ' +
    'named reason, leaving the rest of the group alone. A gRPC result carries grpc: status code (0 is OK ' +
    'and is NOT the refusal sentinel — status 0 with an error and no grpc detail is a refusal), method, ' +
    'redacted metadata. A websocket result carries websocket: transcript, stop_reason (count, timeout, ' +
    'bytes, closed, error) and truncated, true for the three that cut a session short. Frame contents are ' +
    'recorded only when websocket.includePayloads is on.',
  'ASSERTING ON A TRANSPORT: on a websocket result res.getBody() IS the transcript array and ' +
    'res.getStatus() is ALWAYS 0 — a session has no status, so a test on it asserts a constant and cannot ' +
  ...
  'REPORT FILES: report also writes the run to disk — JUnit XML for CI, HTML for a person — at a path ' +
    'inside the collection; what was written comes back under reports.',
  `${SSRF_CONTRACT}`,
].join(' ');
    ...
    'run_collection',
    {
      ...
      inputSchema: {
        ...
        websocket: z.object({
          ...
          idleTimeoutMs: z.number().int().min(0).max(60000, 'idleTimeoutMs must be at most 60000').optional().default(1500).describe('End a session after this much silence, in milliseconds. Default: 1500. Reports stop_reason "idle", which is NOT truncated, because no cap bit. The clock starts at the first frame, so a listen-only request that authors no messages still waits out maxDurationMs. Set 0 to wait for the ceiling, which is what a protocol whose gaps are longer than this needs.'),
          sendIntervalMs: z.number().int().min(0).max(60000, 'sendIntervalMs must be at most 60000').optional().default(0).describe('Wait this long between the messages a request sends, in milliseconds. Default: 0, which sends them all in one tick — right for a stream, useless for a protocol that answers. With a gap the transcript carries the replies interleaved with the sends, at the offsets they arrived. maxDurationMs has to cover the whole paced sequence, and a session stopped part way through reports which messages never went out, by name. The idle bound is not armed while the sequence is still going out, so sendIntervalMs above idleTimeoutMs is safe.'),
          includePayloads: z.boolean().optional().default(false).describe('Record frame CONTENTS, not just direction, timing and size. Default: false, and the default is a security property: outbound frames are recorded after {{var}} interpolation, so a transcript that included them by default would write every secret you passed in variables into a result that is returned by default. A test script does NOT need it: res.getBody() always carries the real payloads. So includePayloads=false with content assertions is the intended CI shape, not a workaround.'),
        ...
        }).strict().optional().describe('Write the run to a file as well as returning it here. Name at least one format. Paths are CONFINED TO THE COLLECTION: a path resolving outside it is refused, with the reason as a run warning, because writing wherever a caller points is a far bigger authorization than running its requests. Missing parent directories inside the collection are created, and an existing report is overwritten. Each file written is reported under reports with its absolute path and size; a report that could not be written never fails the run, it adds a warning naming the format and the reason. A report holds what the results hold — response bodies included, response headers masked exactly as they are here — so it lands on disk with whatever the run saw.'),
        cookieJar: z.boolean().optional().default(true).describe('Keep cookies from each response and send them on later requests in the same run, so a login carries into the requests after it. Scoped to the group — nothing crosses from one group to another, at any parallel setting — held in memory, never written to disk, and matched by host/path/expiry, so a cookie set by one host is not sent to another. Precedence, per cookie name: a Cookie header the request writes itself WINS over the jar, which only adds names the request did not set. This diverges from Bruno\'s CLI, where the stored value wins the clash; a warning names any cookie whose stored value was dropped. Default: true. Set false to send only the Cookie headers a request writes itself.')
      }
    },
    async (args) => {
      try {
        ...
        const result = await RequestExecutor.executeCollection(
          args.collectionPath,
          {
            ...
            includeResponseHeaders: args.includeResponseHeaders,
