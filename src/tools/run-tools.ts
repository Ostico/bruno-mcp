    ...
    'run_collection',
    {
      ...
      inputSchema: {
        ...
        websocket: z.object({
          ...
          includePayloads: z.boolean().optional().default(false).describe('Record frame CONTENTS, not just their direction, timing and size. Default: false, and the default is a security property: outbound frames are recorded after {{var}} interpolation, so a transcript that included them by default would write every secret you passed in variables into a result that is returned by default. Turn it on when you need to assert on what came back.'),
